...
# Converts between Rupees (₹) and Dollars ($)
...
class CurrencyConverter:
    ...
    def __init__(self, master):
        ...
        # Variables for toggling between INR and DLR
        ...
        header = Label(content, text = "Rate at:")
        ...
        eurosButton = Radiobutton(radioFrame, text = "₹ -> $", variable = self.cur, value = 1, command = self.locale)
        eurosButton.grid(row = 1, column = 0)

        poundsButton = Radiobutton(radioFrame, text = "$ -> ₹", variable = self.cur, value = 77, command = self.locale)
